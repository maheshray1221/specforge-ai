              ...
              {integrations.map((integration) => (
                <div key={integration.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <Button size="sm" variant="outline" disabled={loading === `integration-execute-${integration.id}`} onClick={() => void onExecute(integration, "SEND_TEST", integration.provider !== "WEBHOOK" && integration.provider !== "SLACK" && integration.provider !== "GITHUB")}>
                      {loading === `integration-execute-${integration.id}` ? <><Spinner /> Running</> : "Test"}
                    </Button>
                    <Button size="sm" variant="outline" disabled={loading === `integration-execute-${integration.id}`} onClick={() => void onExecute(integration, "EXPORT_TASKS", integration.provider !== "WEBHOOK" && integration.provider !== "SLACK" && integration.provider !== "GITHUB")}>
                      Export tasks
                    </Button>
