            ...
            <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
              ...
              {selectedRequirement && (
                <div className="space-y-4">
                  <Card><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><CardTitle>{selectedRequirement.title}</CardTitle><CardDescription>Latest version {selectedRequirement.versions[0]?.versionNumber ?? 1}</CardDescription></div><div className="flex flex-wrap gap-2"><EditRequirementDialog requirement={selectedRequirement} onUpdated={(updated) => { setRequirements((current) => current.map((item) => item.id === updated.id ? updated : item)); setAnalysis(null); }} /><Button onClick={() => runAnalysis(false)} disabled={action === "analyze"}>{action === "analyze" ? <><Spinner /> Analyzing</> : <><Sparkles className="h-4 w-4" /> Analyze</>}</Button></div></div></CardHeader><CardContent><div className="whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-700">{selectedRequirement.currentContent}</div></CardContent></Card>
                  {analysis?.status === "COMPLETED" && <ClarificationAnswerCard analysis={analysis} requirement={selectedRequirement} loading={action === "clarifications"} onSubmit={saveClarificationAnswers} />}
                </div>
              )}
            </div>
          )}
