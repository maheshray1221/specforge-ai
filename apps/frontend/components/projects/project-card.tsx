...
export function ProjectCard({ project, index, canArchive = false, onUpdated, onArchived }: ProjectCardProps) {
  return (
      ...
      <Card className="group h-full p-5 transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-lg hover:shadow-slate-200/50">
        <div className="flex items-start justify-between gap-3">
          <Link href={`/dashboard/projects/${project.id}`} className="min-w-0 flex-1">
            ...
            <h3 className="mt-4 truncate text-lg font-semibold tracking-tight">{project.name}</h3>
          </Link>
          <div className="flex items-center gap-1">
            {onUpdated && onArchived ? <EditProjectDialog project={project} canArchive={canArchive} onUpdated={onUpdated} onArchived={onArchived} /> : null}
            <Link href={`/dashboard/projects/${project.id}`} aria-label={`Open ${project.name}`} className="rounded-xl bg-slate-50 p-2 text-slate-400 transition group-hover:bg-sky-50 group-hover:text-sky-600"><ArrowUpRight className="h-4 w-4" /></Link>
          </div>
        </div>
